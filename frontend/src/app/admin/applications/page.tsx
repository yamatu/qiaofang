...
export default function ApplicationsPage() {
  ...
  const openEdit = (item: App) => {
    setEditing(item);
    setForm({ title: item.title, description: item.description, image_url: item.image_url, sort_order: item.sort_order });
    setOpen(true);
  };
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-900">应用领域管理</h1>
        <button onClick={() => { setEditing(null); setForm({ title: '', description: '', image_url: '', sort_order: 0 }); setOpen(true); }}
        ...
        {items.map((item, i) => (
          <motion.div key={item.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
            ...
            <div className="w-24 h-16 bg-gray-100 rounded-lg overflow-hidden flex-shrink-0">
              {item.image_url ? <img src={`${API_BASE_URL}${item.image_url}`} alt={item.title} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-gray-300 text-xs">无图</div>}
            </div>
            ...
            <div className="flex gap-2">
              <button onClick={() => openEdit(item)} className="p-2 text-gray-400 hover:text-blue-600"><Pencil size={18} /></button>
      ...
      <Dialog.Root open={open} onOpenChange={setOpen}>
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-2xl p-6 w-full max-w-lg z-50">
            <Dialog.Title className="text-xl font-bold mb-4">{editing ? '编辑' : '新增'}应用领域</Dialog.Title>
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  {form.image_url && <img src={`${API_BASE_URL}${form.image_url}`} className="w-16 h-12 object-cover rounded" />}
                </div>
              </div>
              <div>
